...
class CaptchaFilter(
    ...
    filterConfigManager.getFilterURL(CaptchaFilter::class.java)
) {
    ...
    override fun attemptAuthentication(
        ...
        try {
            val loginByCaptchaRequest = objectMapper.readValue(
                request.inputStream,
                LoginByCaptchaRequest::class.java
            )
            validatorManager.validate(loginByCaptchaRequest)
            return authenticationManager.authenticate(
                CaptchaAuthenticationToken(
                    loginByCaptchaRequest.email, loginByCaptchaRequest.captcha
                )
            )
